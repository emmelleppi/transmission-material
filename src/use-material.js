import { MeshPhysicalMaterial, BackSide, FrontSide, Color, ShaderMaterial, WebGLRenderTarget, RGBFormat, LinearFilter, Vector2, RepeatWrapping } from 'three'
import { SavePass, BlurPass, LambdaPass, RenderPass } from 'postprocessing'
import { useEffect, useMemo } from 'react'
import { vert as vertexShader } from './glsl/transmission-material/vertex.js'
import { frag as fragmentShader } from './glsl/transmission-material/fragment.js'
...
export default function useMaterial(props) {
  ...
  const pipeline = useMemo(() => {
    const frontFbo = new SavePass(new WebGLRenderTarget(size.width, size.height, renderTargetParams))
    const backFbo = new SavePass(new WebGLRenderTarget(size.width, size.height, renderTargetParams))
    ...
    const transmissionMaterial = new ShaderMaterial({
      ...
      uniforms: {
        ...
        resolution: { value: new Vector2(size.width, size.height).multiplyScalar(gl.getPixelRatio()) },
  ...
  }, [gl, noise, gloss, camera, _frontMaterial, _backMaterial, _transmissionMaterial])
